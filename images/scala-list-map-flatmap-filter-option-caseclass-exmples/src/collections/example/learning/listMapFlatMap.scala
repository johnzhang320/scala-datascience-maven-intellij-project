package collections.example.learning
/**
...
object listMapFlatMap {
  def main(args:Array[String]) {
      ...
      println("\n-------origin list --------")
      l.foreach(x=>print(x+" "))
      println("\n-------Using Map anonymous function to time each element by 2 and then create new list----")
      val l2 = l.map(x => x*2)
      l2.foreach ( x => print(x+" ") )
      println("\n------Using function and function inside of Map to change each element and then create new list----")
      ...
      println("\n-------Using Map method to  new list----")
  
      val m = Map(1->2,2->4,3->6)
      m.toList.foreach(x=>print(x+" "))  //(1,2) (2,4) (3,6)

      println("\n-------Using flatMap to print out map key(e._1)list for map----")
      m.flatMap(e => List(e._1)).foreach(x => print(x + " "))
      println("\n-------Using flatMap to print out map value (e._2) list for map----")
